...
public class CheckPointBean implements Serializable {
    ...
    public void selectY(String y) {
        try {
            ...
            // Для нуля используем BigDecimal.ZERO
    ...
    private void persist(BigDecimal x, BigDecimal y, BigDecimal r) {
        ...
        long executionTimeMicroseconds = (endTime - startTime) / 1000; // Преобразуем наносекунды в микросекунды
    ...
    public boolean isHit(BigDecimal x, BigDecimal y, BigDecimal r) {
        ...
        // 1 четверть – четверть окружности радиуса R/2
        ...
        // 3 четверть – прямоугольник: x ∈ [-R/2; 0], y ∈ [-R; 0]
        boolean inRectThird =
        ...
        // 4 четверть – треугольник: x ∈ [0; R], y ∈ [-R; 0], y >= x - R
