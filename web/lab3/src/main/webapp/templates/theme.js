(function () {
  ...
  function updateStartButtonLogo() {
    // Обновляем CSS переменную на корневом элементе для всех .start-button
    const isDark = root.getAttribute('data-theme') === 'dark';
    const contextPath = window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/'));
  ...
  function applyTheme(mode, persist, skipSound) {
    ...
    updateLogoImage(); // Обновляем логотип при смене темы
    updateStartButtonLogo(); // Обновляем логотип на кнопке стартовой страницы
  ...
  function toggleTheme() {
    ...
    // Проверяем заряд батареи при попытке переключить на светлую тему
    ...
    batteryOverrideActive = false; // Сбрасываем переопределение при ручном переключении
  }
  ...
  function enforceBatteryTheme() {
    ...
    if (isLow) {
      ...
      if (currentTheme !== BATTERY_THEME.lowTheme) {
        // Сохраняем текущую тему как ручную, если она была установлена вручную
        if (!batteryOverrideActive && lastManualTheme !== currentTheme) {
          lastManualTheme = currentTheme;
        }
        applyTheme(BATTERY_THEME.lowTheme, false, true); // Настроить звук при автоматической смене
      }
      batteryOverrideActive = true;
      return;
    }

    // Если заряд выше порога и было переопределение, возвращаем последнюю ручную тему
  ...
  function initBatteryTheme() {
    ...
    navigator.getBattery().then((battery) => {
      const update = () => {
        ...
        // Применяем автоматическое переключение при изменении состояния заряда
        enforceBatteryTheme();
      };
  ...
  applyTheme(preferredTheme(), true, true); // Пропускаем звук при инициализации
  initBatteryTheme();
  updateLogoImage(); // Инициализируем логотип при загрузке страницы
  updateStartButtonLogo(); // Инициализируем логотип на кнопке стартовой страницы
})();
