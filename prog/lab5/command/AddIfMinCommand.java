...
/**
 * Команда для добавления нового элемента в коллекцию, если его цена меньше минимальной
 */
public class AddIfMinCommand implements Command {
    ...
    public void execute(String args) {
        if (!args.isEmpty()) {
            outputManager.printError("Команда 'add_if_min' не принимает аргументов");
            return;
        }

        outputManager.println("Добавление нового билета (если его цена минимальна):");
        Ticket newTicket = new Ticket();

        // Заполнение полей билета (аналогично AddCommand)
        setName(newTicket);
        setCoordinates(newTicket);
        setPrice(newTicket);
        setComment(newTicket);
        setRefundable(newTicket);
        setTicketType(newTicket);
        setVenue(newTicket);

        // Находим минимальную цену в коллекции
        int minPrice = collectionManager.getCollection().stream()
                .mapToInt(Ticket::getPrice)
                .min()
                .orElse(Integer.MAX_VALUE);

        // Сравниваем цену нового билета с минимальной
        if (newTicket.getPrice() < minPrice) {
            collectionManager.add(newTicket);
            outputManager.println("Билет успешно добавлен в коллекцию (ID: " + newTicket.getId() + ")");
            outputManager.println("Новая минимальная цена: " + newTicket.getPrice());
        } else {
            outputManager.printError("Цена билета (" + newTicket.getPrice() +
                    ") не меньше минимальной в коллекции (" + minPrice + ")");
        }
    }

    // Методы для заполнения полей билета (аналогичны AddCommand)
