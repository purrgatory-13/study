...
public class AddCommand implements Command {
    ...
    public void execute(String args) {
        if (!args.isEmpty()) {
            outputManager.printError("Команда 'add' не принимает аргументов");
            return;
        }

        outputManager.println("Добавление нового билета:");
        ...
        setPrice(ticket);
        setComment(ticket);
        setRefundable(ticket);
        setTicketType(ticket);
        setVenue(ticket);

        collectionManager.add(ticket);
        outputManager.println("Билет успешно добавлен в коллекцию (ID: " + ticket.getId() + ")");
    }
